...
**CAPPER_ALIGNED — a second qualifying path:** Pure price-inefficiency detection against Pinnacle is necessarily thin (Pinnacle is efficient by design, so it rarely misprices) — many days produce zero qualifying edges even when a trusted outside handicapper has real plays. To capture that without having Claude fabricate its own "who wins" analysis (which it has no real data to support — no injury reports, no bullpen usage, no live form), a handicapper reference pick can independently qualify a bet if: it matches an exact line in today's data (same team/total, same direction), true probability is ≥ 50% (same floor as everywhere else), and EV is no worse than -5% (a sanity check against a bad price, not a requirement that the price be favorable). The handicapper supplies the win-probability judgment; our math only screens out a clearly bad price. These picks are tagged `signal: "CAPPER_ALIGNED"` and tracked separately in the SCORE tab's Signal breakdown, so over time we can see empirically whether this path actually performs — rather than assuming it does.
