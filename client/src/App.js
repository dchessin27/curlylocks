...
function SettingsPanel({ bankroll, unitPct, onSave, onResetAll }) {
  ...
  const [capperInput,  setCapperInput]  = useState("");
  const [capperStatus, setCapperStatus] = useState(null); // null | "saving" | "saved" | "error"
  ...
  async function submitCapperPicks() {
    const picks = capperInput.trim();
    if (!picks) return;
    setCapperStatus("saving");
    try {
      const r = await fetch("/api/capper", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ picks }),
      });
      if (!r.ok) throw new Error("Server error");
      setCapperStatus("saved");
      setTimeout(() => setCapperStatus(null), 4000);
    } catch {
      setCapperStatus("error");
      setTimeout(() => setCapperStatus(null), 3000);
    }
  }
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      ...
      <div style={{ background:"#08140f", border:"1px solid #0f2e22", borderRadius:10, padding:"16px 18px" }}>
        <div style={{ fontSize:9, color:"#cef17b", letterSpacing:1, marginBottom:8 }}>TODAY'S REFERENCE PICKS</div>
        <div style={{ fontSize:9, color:"#3f6b58", marginBottom:10, lineHeight:1.8 }}>Paste today's plays from your sharp handicapper service — one per line. Claude will use them as market context when generating picks.</div>
        <textarea
          value={capperInput}
          onChange={e => setCapperInput(e.target.value)}
          placeholder={"ROCKIES CUBS OVER 10\nCARDINALS ML\nMARLINS +1.5"}
          rows={5}
          style={{ width:"100%", background:"#071210", border:"1px solid #194232", borderRadius:6, color:"#cdedb3", padding:"10px 12px", fontSize:12, fontFamily:"'DM Mono',monospace", outline:"none", resize:"vertical", lineHeight:1.8 }}
        />
        <button
          onClick={submitCapperPicks}
          disabled={!capperInput.trim() || capperStatus === "saving"}
          style={{ width:"100%", marginTop:8, background:capperStatus==="saved"?"#22ff9914":capperStatus==="error"?"#ff554414":"#cef17b14", color:capperStatus==="saved"?"#22ff99":capperStatus==="error"?"#ff5544":"#cef17b", border:`1px solid ${capperStatus==="saved"?"#22ff9944":capperStatus==="error"?"#ff554444":"#cef17b44"}`, borderRadius:6, padding:"9px 0", cursor:(!capperInput.trim()||capperStatus==="saving")?"not-allowed":"pointer", fontSize:9, letterSpacing:1, fontFamily:"'DM Mono',monospace", opacity:!capperInput.trim()?0.4:1 }}>
          {capperStatus==="saving" ? "SAVING..." : capperStatus==="saved" ? "✓ SUBMITTED — Claude will reference these" : capperStatus==="error" ? "✗ FAILED — TRY AGAIN" : "SUBMIT REFERENCE PICKS"}
        </button>
      </div>
